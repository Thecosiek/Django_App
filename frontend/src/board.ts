...
interface InitialBoard {
  ...
  rows: number;
  cols: number;
  dots: Array<{ row: number; col: number; color: string }>;
}

declare global {
  interface Window {
    INITIAL_BOARD: InitialBoard | null;
  }
}
...
let state: BoardState = { rows: 0, cols: 0, dots: [] };
...
const colorBtns = Array.from(document.getElementsByClassName('color-btn')) as HTMLButtonElement[];

genBtn.addEventListener('click', () => {

  const rowsInput = form.querySelector('[name="rows"]') as HTMLInputElement;
  const colsInput = form.querySelector('[name="cols"]') as HTMLInputElement;
...
function updateDotsInDOM(): void {
  ...
  state.dots.forEach(d => {
    ...
    const cell = gridContainer.querySelector(selector) as HTMLElement;
    if (cell) {
...
document.addEventListener('DOMContentLoaded', () => { // nadpisane
form.addEventListener('submit', async (e) => {
  ...
  if (initial) {
    (form.querySelector('[name="name"]') as HTMLInputElement).value = initial.name;
    (form.querySelector('[name="rows"]')  as HTMLInputElement).value = initial.rows.toString();
    (form.querySelector('[name="cols"]')  as HTMLInputElement).value = initial.cols.toString();
