...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const mapImg = document.getElementById('bg-image');
    const overlayElement = document.getElementById('overlay');
    if (!(overlayElement instanceof SVGSVGElement)) {
        throw new Error('Element #overlay is not an SVGSVGElement');
    }
    const overlay = overlayElement;
    const formPrefix = document.getElementById('id_' + 'form-TOTAL_FORMS').name.split('-')[0];
    let formCount = parseInt(document.getElementById(`id_${formPrefix}-TOTAL_FORMS`).value);
    ...
    function renderRoute() {
        ...
        const points = [];
        document.querySelectorAll('.point-form').forEach(div => {
            const del = div.querySelector('input[name$="-DELETE"]');
            if (del && del.checked)
                return;
            const xInput = div.querySelector('input[name$="-x"]');
            const yInput = div.querySelector('input[name$="-y"]');
            const x = parseFloat((xInput === null || xInput === void 0 ? void 0 : xInput.value) || '');
            const y = parseFloat((yInput === null || yInput === void 0 ? void 0 : yInput.value) || '');
            if (!isNaN(x) && !isNaN(y)) {
                points.push({
                    x,
                    y,
    ...
    function addForm(x = null, y = null) {
        const idx = formCount;
    ...
    `;
        ...
        formCount++;
        document.getElementById(`id_${formPrefix}-TOTAL_FORMS`).value = formCount.toString();
        renderRoute();
    }
    ...
    container.addEventListener('click', e => {
        if (e.target.matches('.btn-remove-point')) {
            const div = e.target.closest('.point-form');
    ...
    overlay.addEventListener('click', e => {
        ...
        if (el.matches('.point-circle')) {
            ...
            const div = container.querySelector(`.point-form[data-form-index="${idx}"]`);
            const del = div.querySelector('input[type="checkbox"][name$="-DELETE"]');
            if (del)
                del.checked = true;
            div.style.display = 'none';
    ...
    overlay.addEventListener('mousemove', e => {
        if (draggingCircle) {
            ...
            const xInput = document.querySelector(`input[name="${formPrefix}-${formIndex}-x"]`);
            const yInput = document.querySelector(`input[name="${formPrefix}-${formIndex}-y"]`);
            if (xInput && yInput) {
